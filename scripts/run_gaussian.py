...
from posteriors import gaussian
from postprocessing import processEmceeChain 

#TODO: 5 dimensions - add Bq
...
# Model
model = numinjections + "pop" + pop + "_gaussian"
...
if len(old_chains)==0:
    ...
    # Initialize walkers
    initial_mu_chis = draw_initial_walkers_uniform(nWalkers, (-0.2,0.2))
    initial_sigma_chis = draw_initial_walkers_uniform(nWalkers, (0.17,0.25))
    initial_mu_costs = draw_initial_walkers_uniform(nWalkers, (-0.2,0.2))
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        gaussian, # model in posteriors.py
