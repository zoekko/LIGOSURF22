...
from posteriors import binned
from postprocessing import processEmceeChain 
#TODO: add another dimension - add Bq

pop = '1'
numinjections = '70'

# x_bins = [-1, -0.7, -0.4, -0.2, -0.1, 0, 0.1, 0.2, 0.4, 0.7, 1]

chi_x_bins = [0, 0.2, 0.4, 0.6, 0.8, 1]
cost_x_bins = [-1, -0.5, 0, 0.5, 1]

chi_num_bins = len(chi_x_bins) - 1
cost_num_bins = len(cost_x_bins) - 1

intial_walkers_size = chi_num_bins + cost_num_bins - 2
...
# Model
model = numinjections + "pop" + pop + "_binned"
...
priorDict = {
    'chibin1':(0., 0.2),
    'chibin2':(0., 0.2), # the sigma^2 bounds are [0.005, 0.25], so sigma goes from [sqrt(0.005), 4]  
    'chibin3':(0., 0.2),
    'chibin4':(0., 0.2), 
    'cosbin1':(0., 0.2),
    'cosbin2':(0., 0.2),
...
if len(old_chains)==0:
    ...
    total_bins = []
    for i in np.arange(intial_walkers_size):
        a_bin = np.random.random(nWalkers)*(bin_max-bin_min)+bin_min
        total_bins += [a_bin]
        
    initial_Bqs = np.random.normal(loc=0, scale=3, size=nWalkers)
    total_bins += [initial_Bqs]

    initial_walkers = np.transpose(total_bins)
...
results = {
    'chibin1':{'unprocessed':chainRaw[:,:,0].tolist(), 'processed':chainDownsampled[:,0].tolist()},
    'chibin2':{'unprocessed':chainRaw[:,:,1].tolist(), 'processed':chainDownsampled[:,1].tolist()},
    'chibin3':{'unprocessed':chainRaw[:,:,2].tolist(), 'processed':chainDownsampled[:,2].tolist()},
    'chibin4':{'unprocessed':chainRaw[:,:,3].tolist(), 'processed':chainDownsampled[:,3].tolist()},
    'costbin1':{'unprocessed':chainRaw[:,:,4].tolist(), 'processed':chainDownsampled[:,4].tolist()},
    'costbin2':{'unprocessed':chainRaw[:,:,5].tolist(), 'processed':chainDownsampled[:,5].tolist()},
    'costbin3':{'unprocessed':chainRaw[:,:,6].tolist(), 'processed':chainDownsampled[:,6].tolist()},
