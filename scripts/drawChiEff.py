...
# TODO: pass all the spike stuff as none

def drawChieffsBeta(mu_chi, sigma_chi, MF_cost, sigma_cost, f_spike, sigma_spike, cost_min, Bq, n=1): 
...
def drawChieffsGauss(mu_chi, sigma_chi, mu_cost, sigma_cost, cost_min, Bq, n=1): 
    ...
    ----------
    mu_chi : float
        mean of spin magnitude gaussian distribution
    sigma_chi : float 
        std. dev. of spin magnitude beta distribution 
    mu_cost : float
        mean of cos tilt angle gaussian distribution
    sigma_cost : float
        std. dev. of aligned spin subpopulation for cos tilt angle distribution
    frac_in_spike : float or None
        mixing fraction in half gaussian spike at spin mag. = 0
...
def drawChieffsBinned(chibins, costbins, chi_xbins, cost_xbins, Bq, n=1): 
    ...
    ----------
    chibins : list
        bins corresponding to chi effective
    costbins : list
        bins corresponding to cos theta
    chi_xbins : list
        x bins corresponding to chi effective
    cost_xbins : list
        x bins corresponding to cos theta
    Bq : float
        power law slope of the mass ratio distribution 
    n : int
        number of chi-eff samples to draw from the distribution
    ...
    p_chi1 = calculate_Bins(chi1s, chibins, chi_xbins)
    p_chi2 = calculate_Bins(chi2s, chibins, chi_xbins)
    p_cost1 = calculate_Bins(cost1s, costbins, cost_xbins)
    p_cost2 = calculate_Bins(cost2s, costbins, cost_xbins)
    p_masses = p_astro_masses(m1s, m2s, bq=Bq)
