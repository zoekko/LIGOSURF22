...
def merge_dicts(dicts):
    '''given a list of dictionaries, merge them (removes duplicates)'''
    final = {}
    for d in dicts:
        final = {**final, **d}
    return final
...
def calculate_Bins(x, bins, x_bins):
    ...
    for i in np.arange(length - 1):
        ...
        next_bin = x_bins[i + 1]
        if i == (length - 2):
            bin_i = np.where((x >= curr_bin) & (x <= next_bin))
        else:
            bin_i = np.where((x >= curr_bin) & (x < next_bin))
        y[bin_i] = bins[i] / (next_bin - curr_bin)
